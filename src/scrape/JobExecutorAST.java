...
public class JobExecutorAST {
	...
	public static void executeJob(int jobID, String searchTerm){
		...
		try{
		Iterator<String> results  = FetchProductListingFromSearchAST.FetchSearch(searchTerm).iterator();
		int i=0;
		while(results.hasNext()){
			...
			try{
				HashMap<String,List<String>> cleanedHashMap = FetchProductListingFromSearchAST.initialClean(FetchProductListingFromSearchAST.getProductInfo(url));
	...
	public static void executeJobUsingDatasetOld(int jobID, HashMap<String,String> dataSet){
		...
		try{
		Iterator<String> results  = FetchProductListingFromSearchAST.FetchSearch(dataSet.get("Type/Dimension | EF001139")).iterator();
		int i=0;
		while(results.hasNext()){
			i++;
			String url = results.next();
			...
			individualResultMap.put("status", "success");
			try{
				HashMap<String,List<String>> cleanedHashMap = ProductMatching.matchToDataFile(FetchProductListingFromSearchAST.initialClean(FetchProductListingFromSearchAST.getProductInfo(url)),dataSet);
				NewFileWriter.writeFile(SupportFileReader.prettyJSON(JSONObject.toJSONString(individualResultMap))+"\n--\n"+SupportFileReader.prettyJSON(JSONObject.toJSONString(cleanedHashMap)), ""+jobID+"-"+i+".job");
				individualResults.add(""+jobID+"-"+i);
	...
	public static void executeJobUsingDataset(int jobID, HashMap<String,String> dataSet){
		...
		for(String searchTerm : searchTerms)  {
			...
			results.addAll(FetchProductListingFromSearchAST.FetchSearch(searchTerm));
			}
			}	
