...
public class ProductMatchingAST {
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
			...
			for(int i=0;i<WORKER_NUMBER;i++){
				Thread T1 = new Thread(new JobExecutorRunnableAST(dataEntries.subList(((int)Math.floor(i*(dataEntries.size()/4.0))),((int)Math.floor((i+1)*(dataEntries.size()/4.0)))),((int)Math.floor(i*(dataEntries.size()/4.0)))));
			      T1.start();
			      workers[i]=T1;
			}
			...
			for(int i =0; i<dataEntries.size();i++){
				JobExecutorAST.executeJobUsingDataset(i+1, (HashMap<String, String>) dataEntries.get(i));
				jobs[i]=""+(i+1);
			}}
